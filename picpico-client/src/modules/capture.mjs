export const Capture = () => {
  ...
  if (!gl) {
    console.log("there is no fimCanvas");
    return;
  }
  // setup glsl program
  ...
  const shutter = document.getElementById("takePicBtn");
  shutter.addEventListener("click", () => {
    drawScene();
    filmCanvas.toBlob(blob => {
      saveBlob(blob, `screencapture-${filmCanvas.width}*${filmCanvas.height}.png`);
    });
  });
